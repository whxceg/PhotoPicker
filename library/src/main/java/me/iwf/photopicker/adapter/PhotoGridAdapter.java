...
public class PhotoGridAdapter extends SelectableAdapter<PhotoGridAdapter.PhotoViewHolder> {
    ...
    public void onBindViewHolder(final PhotoViewHolder holder, int position) {
        ...
        if (getItemViewType(position) == ITEM_TYPE_PHOTO) {
            ...
            if (canLoadImage) {
                glide.load(new File(photo.getPath())).apply(new RequestOptions().centerCrop()
                        .override(imageSize, imageSize).placeholder(R.drawable.__picker_ic_photo_black_48dp).error(R.drawable.__picker_ic_broken_image_black_48dp)).thumbnail(0.5f).into(holder.ivPhoto);
            }
